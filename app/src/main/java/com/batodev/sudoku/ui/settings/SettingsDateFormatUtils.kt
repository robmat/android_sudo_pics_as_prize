...
internal val DateFormats = listOf(
    "",
    "dd/MM/yy",
    "dd.MM.yy",
    "MM/dd/yy",
...
internal fun buildDateFormatEntries(): Map<String, String> {
    return DateFormats.associateWith { dateFormatEntry ->
        val dateString = ZonedDateTime.now().format(
            when (dateFormatEntry) {
                ...
                else -> DateTimeFormatter.ofPattern(dateFormatEntry)
            }
        )
...
internal fun buildCustomDateFormatText(dateFormat: String): String {
    return if (!DateFormats.contains(dateFormat)) {
        "$dateFormat (${
