...
private fun GameViewModel.revalidateErrorCell(
...
private fun GameViewModel.applyMistakeChecking(
    board: List<List<Cell>>,
    row: Int,
    col: Int,
): List<List<Cell>> =
    when (mistakesMethod.value) {
        ...
        2 -> {
            isValidCell(board, board[row][col])
        }
...
private fun GameViewModel.handleMistakeLimitIfError(
    board: List<List<Cell>>,
    row: Int,
    col: Int,
) {
