...
internal fun DrawScope.drawCellsToHighlight(cellsToHighlight: List<Cell>?, highlightColor: Color, cellSize: Float) {
...
internal fun DrawScope.drawCrossHighlight(size: Int, highlightColor: Color, cellSize: Float) {
...
internal data class DrawNumbersOptions(val highlightErrors: Boolean, val questions: Boolean)
        ...
        for (i in 0 until size) {
            for (j in 0 until size) {
                if (board[i][j].value != 0) {
                    ...
                    val textToDraw = if (options.questions) {
                        "?"
                    } else {
...
internal data class NoteCellMetrics(val cellSize: Float, val cellSizeDivWidth: Float, val cellSizeDivHeight: Float)
        ...
        notes.forEach { note ->
            ...
            canvas.nativeCanvas.drawText(
                ...
                note.col * metrics.cellSize + metrics.cellSizeDivWidth / 2f + (
                    metrics.cellSizeDivWidth * getNoteRowNumber(
                        note.value,
                        size
                    )
                    ) - noteTextMeasure / 2f,
                note.row * metrics.cellSize + metrics.cellSizeDivHeight / 2f + (
                    metrics.cellSizeDivHeight * getNoteColumnNumber(
                        note.value,
                        size
                    )
