...
internal fun GameViewModel.addNote(note: Int, row: Int, col: Int): List<Note> {
    return notes.plus(Note(row, col, note))
}

internal fun GameViewModel.removeNote(note: Int, row: Int, col: Int): List<Note> =
...
internal fun GameViewModel.autoEraseNotes(board: List<List<Cell>> = getBoardNoRef(), cell: Cell): List<Note> {
