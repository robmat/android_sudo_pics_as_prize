 ...
 * cell resides.
 */
internal fun cellToColumnInternal(cell: Int): Int = cell % QQWing.ROW_COL_SEC_SIZE

/**
 * Given the index of a cell (0-80) calculate the row (0-8) in which it
 * resides.
 */
internal fun cellToRowInternal(cell: Int): Int = cell / QQWing.ROW_COL_SEC_SIZE
...
internal fun cellToSectionInternal(cell: Int): Int =
    cell / QQWing.SEC_GROUP_SIZE * QQWing.GRID_SIZE_ROW + cellToColumnInternal(cell) / QQWing.GRID_SIZE_COL
...
internal fun cellToSectionStartCellInternal(cell: Int): Int =
    cell / QQWing.SEC_GROUP_SIZE * QQWing.SEC_GROUP_SIZE +
 ...
 */
internal fun rowToFirstCellInternal(row: Int): Int = QQWing.ROW_COL_SEC_SIZE * row

/**
 * Given a column (0-8) calculate the first cell (0-80) of that column.
 */
internal fun columnToFirstCellInternal(column: Int): Int = column

/**
 * Given a section (0-8) calculate the first cell (0-80) of that section.
 */
internal fun sectionToFirstCellInternal(section: Int): Int =
    section % QQWing.GRID_SIZE_ROW * QQWing.GRID_SIZE_COL +
 ...
 */
internal fun getPossibilityIndexInternal(
    valueIndex: Int,
    cell: Int,
): Int = valueIndex + QQWing.ROW_COL_SEC_SIZE * cell

/**
 * Given a row (0-8) and a column (0-8) calculate the cell (0-80).
 */
internal fun rowColumnToCellInternal(
    row: Int,
    column: Int,
): Int = row * QQWing.ROW_COL_SEC_SIZE + column

/**
 * Given a section (0-8) and an offset into that section (0-8) calculate the
 * cell (0-80)
 */
internal fun sectionToCellInternal(
    section: Int,
    offset: Int,
): Int =
    sectionToFirstCellInternal(section) +
